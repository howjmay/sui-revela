...
use move_compiler::{
    expansion::ast::{
        AbilitySet, Address, Attributes, DatatypeTypeParameter, EnumDefinition, Function,
        FunctionBody_, FunctionSignature, ModuleAccess_, ModuleDefinition, ModuleIdent,
        ModuleIdent_, Program, StructDefinition, StructFields, Type, Type_,
    },
...
fn map_enum(
    ...
    enum_: &move_binary_format::file_format::EnumDefinition,
    naming: &Naming,
) -> Result<EnumDefinition, anyhow::Error> {
    let enum_handle = compiled_module.datatype_handle_at(enum_.enum_handle);

    let type_parameters = enum_handle
        .type_parameters
        .iter()
        .enumerate()
        .map(|(idx, x)| {
    ...
    use move_compiler::expansion::ast::VariantDefinition;
    use move_compiler::parser::ast::VariantName;
    ...
    let variants = enum_
        ...
        .enumerate()
        .map(|(idx, variant)| {
            let name = Symbol::from(compiled_module.identifier_at(variant.variant_name).as_str());
            let name = move_compiler::parser::ast::VariantName(span_(name));

            let fields = UniqueMap::maybe_from_iter(
                variant
                    .fields
                    .iter()
                    .map(|field| {
                        let field_name =
                            Symbol::from(compiled_module.identifier_at(field.name).as_str());
                        let field_name = Field(span_(field_name));
                        Ok((
                            field_name,
                            (0, map_type(compiled_module, &field.signature.0, naming)?),
                        ))
                    })
                    .collect::<Result<Vec<(Field, (usize, Type))>, anyhow::Error>>()?
                    .into_iter(),
            )
            .map_err(|_| anyhow::Error::msg(format!("Duplicate enum field name")))?;

            let is_positional = fields
                .iter()
                .map(|(_, f, _)| f)
                .into_iter()
                .enumerate()
                .all(|(idx, f)| f == &Symbol::from(format!("pos{}", idx)));
            let fields = if is_positional {
                move_compiler::expansion::ast::VariantFields::Positional(
                    fields.into_iter().map(|(_, x)| x.1).collect::<Vec<_>>(),
                )
            } else {
                move_compiler::expansion::ast::VariantFields::Named(fields)
            };

            let definition = move_compiler::expansion::ast::VariantDefinition {
                index: idx,
                loc: fake_loc(),
                fields,
            };
        ...
        .collect::<Result<Vec<(VariantName, VariantDefinition)>, anyhow::Error>>()?;

    let variants =
        UniqueMap::<VariantName, VariantDefinition>::maybe_from_iter(variants.into_iter())
            .map_err(|_| anyhow::Error::msg("Duplicated enum name"))?;

    Ok(EnumDefinition {
        attributes: UniqueMap::new(),
        loc: fake_loc(),
        abilities: map_abilities(enum_handle.abilities).unwrap(),
        type_parameters,
        variants,
        warning_filter: move_compiler::diagnostics::WarningFilters::new_for_source(),
        index: enum_.enum_handle.0 as usize,
    })
}
...
fn create_dummy_for_non_existing_modules(
    ...
    for (&module_id, module) in &dummy_modules {
        ...
        // TODO - do we need to add enums?
        let enums = UniqueMap::<DatatypeName, EnumDefinition>::new();
...
pub(crate) fn create_program(
    ...
    let adding_modules: Vec<_> = binaries.into_iter().map(|binary| binary.clone()).collect();
    ...
    for (module_idx, compiled_module) in adding_modules.iter().enumerate() {
        ...
        let mut enums: UniqueMap<DatatypeName, EnumDefinition> = UniqueMap::new();
        ...
        for enum_ in compiled_module.enum_defs() {
            let enum_handle = enum_.enum_handle;
            let name_idx = compiled_module.datatype_handle_at(enum_handle).name;
            let name_str = compiled_module.identifier_at(name_idx).as_str();
            let name = span_(Symbol::from(name_str));
            ...
            enums
                .add(
                    DatatypeName(name),
                    map_enum(compiled_module, enum_, naming)?,
                )
                .map_err(|(name, _)| anyhow::Error::msg(format!("Error adding enum {}", name)))?;
        }
