    ...
    fn evaluate(
        ...
        match self {
            Operation::Function(mid, fid, types) => {
                ...
                let has_acquire_resources = !func.get_acquires_global_resources().is_empty();
            ...
            Operation::Unpack(mid, sid, types) | Operation::Pack(mid, sid, types) => {
                ...
                match self {
                    Operation::Unpack(..) => Ok(OperationEvaluatorResult {
                        cannot_keep: false,
                        expr: ExprNodeOperation::DatatypeUnpack(
                            name,
                            keys,
                    ...
                    Operation::Pack(..) => {
                        ...
                        Ok(OperationEvaluatorResult {
                            cannot_keep: false,
                            expr: ExprNodeOperation::DatatypePack(
                                name,
                                keys.iter()
                                    .zip(args.iter())
            ...
            Operation::PackVariant(mid, dtid, vid, tys) => {
                let module_env = ctx.func_env.module_env.env.get_module(*mid);
                let enum_env = module_env.get_enum(*dtid);

                let variant = enum_env.get_variant(*vid);

                let enum_name = shortest_name(ctx, mid, enum_name(&enum_env));
                ...
                Ok(OperationEvaluatorResult {
                    ...
                    expr: ExprNodeOperation::VariantPack(
                        ...
                        variant
                            .get_fields()
                            .map(|x| x.get_name().display(enum_env.symbol_pool()).to_string())
                            .zip(args.iter())
                            .map(|(k, x)| (k, x.value_copied()))
                            .collect(),
                        tys.clone(),
                    )
                    .to_expr(),
            ...
            Operation::UnpackVariant(mid, dtid, vid, tys, reftype) => {
                let module_env = ctx.func_env.module_env.env.get_module(*mid);
                let enum_env = module_env.get_enum(*dtid);
            ...
            | Operation::BorrowGlobal(mid, sid, types) => {
                ...
                Ok(OperationEvaluatorResult {
                    ...
                    expr: ExprNodeOperation::Func(
                        format!("{}", func),
                        arg_fn(args)?,
                        vec![Type::Datatype(mid.clone(), sid.clone(), types.clone())],
                    )
                    .to_expr(),
                })
            }
            ...
            Operation::Destroy => Ok(OperationEvaluatorResult {
                cannot_keep: false,
...
pub fn enum_name(enum_env: &move_model::model::EnumEnv<'_>) -> String {
    enum_env
        .get_name()
        .display(enum_env.symbol_pool())
        .to_string()
