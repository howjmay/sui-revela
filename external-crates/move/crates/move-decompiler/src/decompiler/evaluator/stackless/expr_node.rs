...
impl ExprNodeOperation {
    pub fn copy(&self) -> Self {
        match self {
            ...
            ExprNodeOperation::VariantUnpack(name, variant_name, keys, val, types, ref_type) => {
    ...
    fn to_source_with_ctx(
        ...
        match self {
            ...
            ExprNodeOperation::DatatypeUnpack(_name, _keys, _val, _types) => unreachable!(),
            ...
            ExprNodeOperation::VariantUnpack(name, variant_name, keys, val, types, ref_type) => Ok(format!(
                "{}{}::{}/*{}*/{{{}}} = {}",
                name,
                Self::typeparams_to_source(types, naming),
                variant_name,
                match ref_type {
                    move_model::model::RefType::ByValue => "byval",
                    move_model::model::RefType::ByImmRef => "byimmref",
                    move_model::model::RefType::ByMutRef => "bymutref",
                },
                keys.iter()
                    .map(|x| x.to_string())
                    .collect::<Vec<String>>()
    ...
    fn collect_variables(
        ...
        match self {
            ...
            ExprNodeOperation::VariantPack(_, _, args, _)
            ...
            ExprNodeOperation::VariantUnpack(_, _, _, val, _, _)
    ...
    fn commit_pending_variables(&self, variables: &HashSet<usize>) -> ExprNodeRef {
        match self {
            ...
            ExprNodeOperation::VariantPack(name, variant_name, fields, typs) => ExprNodeOperation::VariantPack(
                name.clone(),
                variant_name.clone(),
                fields
                    .iter()
            ...
            .to_node(),
            ExprNodeOperation::VariantUnpack(name, variant_name, fields_names, expr, typs, ref_type) => {
                ExprNodeOperation::VariantUnpack(
                    name.clone(),
                    variant_name.clone(),
                    fields_names.clone(),
...
impl Expr {
    ...
    pub fn is_borrow(&self) -> Option<Expr> {
        match &self.node.borrow().operation {
            ExprNodeOperation::BorrowLocal(inner, ..) => Some(Self::new(inner.borrow().copy_as_ref())),
            _ => None,
        }
    }
