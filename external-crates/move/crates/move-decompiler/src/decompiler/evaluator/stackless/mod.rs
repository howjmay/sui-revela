...
use crate::decompiler::cfg::algo::blocks_stackless::AnnotatedBytecode;
...
pub struct StacklessEvaluationContext<'a> {
    ...
    last_switch_expr: Option<(usize, Expr)>,
    loop_entry: bool,
...
impl<'a> Clone for StacklessEvaluationContext<'a> {
    fn clone(&self) -> Self {
        ...
        Self {
            ...
            last_switch_expr: self.last_switch_expr.clone(),
...
impl<'a> StacklessEvaluationContext<'a> {
    ...
    pub fn shortest_prefix(&self, mod_id: &ModuleId) -> String {
        super::super::utils::naming::shortest_prefix(&self.func_env.module_env, mod_id)
    }
    ...
    pub fn push_switch_variable(&mut self, idx: usize, value: Expr) -> Result<(), anyhow::Error> {
        if self.last_switch_expr.is_some() {
            return Err(anyhow::anyhow!("Switch variable already pushed"));
        }
        self.last_switch_expr = Some((idx, value));
        Ok(())
    }

    pub fn pop_switch_variable(&mut self) -> Option<(usize, Expr)> {
        let expr = self.last_switch_expr.clone();
        self.last_switch_expr = None;
        expr
    }
    ...
    pub fn run(
        ...
        match &bytecode.bytecode {
            ...
            Bytecode::VariantSwitch(..) => Ok(StacklessEvaluationRunResult {
                results: Expr::ignored(),
                new_variables,
                flushed_variables,
                cannot_keep_as_expr: false,
    ...
    ) -> (ModuleId, move_model::model::DatatypeId) {
        match ty {
            move_model::ty::Type::Datatype(mid, dtid, _) => (*mid, *dtid),
            move_model::ty::Type::Reference(_, ty) => self.resolve_datatype(ty),
            _ => panic!("Expected struct type"),
        }
    }

    pub(crate) fn generate_switch_case_match(
        &self,
        ty: &move_model::ty::Type,
        vid: usize,
    ) -> String {
        let (mid, dtid) = self.resolve_datatype(ty);
        let module_env = self.func_env.module_env.env.get_module(mid);
        let enum_env = module_env.get_enum(dtid);

        let variant = enum_env.get_variant_by_tag(vid);

        format!(
            "{}::{}{}",
            shortest_name(self, &mid, enum_name(&enum_env)),
            variant.get_name().display(enum_env.symbol_pool()),
