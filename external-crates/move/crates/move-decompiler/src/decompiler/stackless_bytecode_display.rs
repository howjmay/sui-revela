...
use super::cfg::{self, datastructs::Terminator, StacklessBlockContent, StacklessBlockIdentifier};
        ...
        while let Some((_, code)) = iter.next() {
            if iter.peek().is_none() {
                ...
                match &node.next {
                    ...
                    Terminator::VariantSwitch { targets: _ } => {
                        // no need, switch info will be displayed in last instruction
                    }
