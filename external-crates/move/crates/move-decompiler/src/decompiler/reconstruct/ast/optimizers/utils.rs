    ...
    for item in unit.blocks.iter() {
        match item {
            ...
            DecompiledCodeItem::VariantSwitchStatement {
                ...
                referenced_variables.insert(*local_variable);
    ...
    for item in unit.blocks.iter() {
        match item {
            ...
            DecompiledCodeItem::VariantSwitchStatement {
                ...
                live_variables.insert(*local_variable);
                live_variables.extend(result_variables.iter());
                expr.collect_variables(live_variables, implicit_variables, false, true);
                for case in cases.iter() {
                    collect_live_variables(case, live_variables, implicit_variables);
                }
            }
            DecompiledCodeItem::WhileStatement { body, cond } => {
    ...
    for item in unit.blocks.iter() {
        match item {
            ...
            DecompiledCodeItem::VariantSwitchStatement {
                ...
                result_variables.extend(r.iter());
                for case in cases.iter() {
                    get_variable_declaration_order(case, result_variables);
                }
            }
