...
pub(crate) fn rename_variables(
    ...
    unit.for_case_variable = unit
        .for_case_variable
        .as_ref()
        .map(|x| x.clone_with_variable_remap(&|x| renamed_variables[&x]));
    ...
    for item in unit.blocks.iter_mut() {
        match item {
            ...
            DecompiledCodeItem::VariantSwitchStatement {
                ...
                *local_variable = renamed_variables[local_variable];
                for v in result_variables.iter_mut() {
                    *v = renamed_variables[v];
                }
                expr.rename_variables(renamed_variables);
                for case in cases.iter_mut() {
                    rename_variables(case, renamed_variables);
                }
            }
        }
    }
...
pub(crate) fn process_variable_alias(
    ...
    new_unit.for_case_variable = unit
        .for_case_variable
        .as_ref()
        .map(|x| x.clone_with_variable_remap(&|x| alias.get(&x).unwrap_or(&x).clone()));
    ...
    for item in unit.blocks.iter() {
        match item {
            ...
            DecompiledCodeItem::VariantSwitchStatement {
                ...
                let units = cases
                    .iter()
                    .map(|x| process_variable_alias(x, alias, in_alias, &defined))
                ...
                new_unit
                    .blocks
                    .push(DecompiledCodeItem::VariantSwitchStatement {
                        expr,
                        cases_ref_type: cases_ref_type.clone(),
                        local_variable: alias.get(local_variable).unwrap_or(local_variable).clone(),
                        cases: units,
                        result_variables,
                        use_as_result: use_as_result.clone(),
                    });
