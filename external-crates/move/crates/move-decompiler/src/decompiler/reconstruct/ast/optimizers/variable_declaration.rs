...
fn collect_borrow_checker_violations(
    ...
    for item in unit.blocks.iter() {
        match item {
            ...
            I::VariantSwitchStatement { expr, cases, .. } => {
                ...
                for case in cases.iter() {
                    let mut case_state = state.fork();
                    case_terminated = collect_borrow_checker_violations(
                        case,
                        variable_index,
                        function_target,
                        &mut case_state,
    ...
    for item in unit.blocks.iter() {
        match item {
            ...
            I::VariantSwitchStatement {
                ...
                let new_expr = expr.clone().commit_pending_variables(should_declare);
                let new_cases: Vec<_> = cases
                    .iter()
                    .map(|case| {
                        apply_variable_declaration(case, should_declare, remove_possible_assign)
                    })
                    .collect::<Result<_, _>>()?;
                new_unit.blocks.push(I::VariantSwitchStatement {
                    expr: new_expr,
