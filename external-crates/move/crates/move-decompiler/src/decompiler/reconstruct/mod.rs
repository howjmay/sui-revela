...
impl<'a> SourceGen<'a> {
    ...
    pub(crate) fn generate(
        ...
        if evaluation_ctx.pop_switch_variable().is_some() {
            return Err(anyhow::anyhow!("final switch variable stack not empty"));
        }
        ...
        let mutable_variables_renamed: HashSet<_> = mutable_variables
            .iter()
            .map(|x| *renamed_variables.get(x).unwrap_or(x))
            .collect();

        Ok(unit.to_source(&final_naming, true, &mutable_variables_renamed)?)
    }
    ...
    fn visit_hyperblock(
        ...
        match block.inner() {
            ...
            HyperBlock::VariantSwitchBlocks { cases } => {
                let initial_evaluation_ctx = evaluation_ctx.clone();
                let Some(expr) = evaluation_ctx.pop_switch_variable() else {
                    return Err(anyhow::anyhow!("fail to obtain switch variable"));
                };
                ...
                let vars: Vec<_> = cases
                    ...
                    .map(|unit| {
                        (
                            unit.is_terminated_in_loop(),
                            find_need_propagate_inner_defining_variables(unit, evaluation_ctx),
                        )
                    })
                    .collect();

                let switch_vars: HashSet<_> = vars
                    .clone()
                    .into_iter()
                    .filter(|(terminated, _)| !*terminated)
                    .map(|(_, vars)| vars)
                    .reduce(|acc, x| {
                        if let (Some(x), Some(y)) = (&acc, &x) {
                            Some(x.intersection(y).cloned().collect())
                        } else {
                            None
                        }
                    })
                ...
                let mut need_declares = HashSet::new();
                for vs in &vars {
                    if let (false, Some(vs)) = vs {
                        need_declares.extend(vs.difference(&switch_vars));
                    }
                }
                let mut need_declares = need_declares.iter().cloned().collect::<Vec<_>>();
                need_declares.sort();
                ...
                let mut cases_ctx = cases
                    .iter()
                    .map(|_| evaluation_ctx.clone())
                    .collect::<Vec<_>>();

                let mut cases_s_ctx = cases
                    .iter()
                    .map(|_| {
                        let mut ctx = s_ctx.clone();
                        ctx.enter_block();
                        ctx
                    })
                    .collect::<Vec<_>>();
                ...
                let mut case_var_ref_type = HashSet::new();
                for i in 0..cases.len() {
                    vars[i].1.as_ref().map(|tv| {
                        tv.intersection(&switch_vars)
                            .for_each(|&v| cases_s_ctx[i].need_propagate(v))
                    });
                    let mut unit =
                        self.visit_codeunit(&mut cases_ctx[i], &mut cases_s_ctx[i], &cases[i])?;
                    if let Some(ref_type) = get_case_var_ref_type(&cases[i]) {
                        case_var_ref_type.insert(ref_type);
                    }
                    let ty = self.func_target.get_local_type(expr.0);
                    let case_match_default = evaluation_ctx.generate_switch_case_match(ty, i);
                    unit.as_mut()
                        .set_for_case_variable(expr.0, i, ty.clone(), case_match_default);
                    units.push(unit);
                }
                let mut branches_to_merge = Vec::new();
                for i in 0..cases.len() {
                    if !cases[i].is_terminated() {
                        branches_to_merge.push(&cases_ctx[i]);
                    }
                }
                ...
                let mut result_variables = switch_vars.iter().cloned().collect::<Vec<_>>();
                result_variables.sort();

                if case_var_ref_type.len() > 1 {
                    return Err(anyhow::anyhow!("variant switch with different ref type"));
                }

                let (match_expr, cases_ref_type) = match case_var_ref_type.iter().next() {
                    Some((Some(v), RefType::ByValue)) => {
                        // expr.1 must be ref of v
                        // let expr_reevaluate: crate::decompiler::evaluator::stackless::expr_node::Expr = expr.1.copy();
                        // println!("Before reevaluate: {:?}", expr_reevaluate);
                        // let mut dummy_pending = HashSet::new();
                        // dummy_pending.insert(*v);
                        // let expr_reevaluate = expr_reevaluate.commit_pending_variables(&dummy_pending);
                        // println!("Reevaluate: {:?}", expr_reevaluate);
                        // if expr_reevaluate == Expr::BorrowLocal(Expr)
                        let pop_last_ref = if let Some(DecompiledCodeItem::AssignStatement {
                            variable: a_variable,
                            value: a_value,
                            is_decl: true,
                        }) = codeunit.blocks.last()
                        {
                            // Rewrite:
                            // let var = &x;  | ==== |
                            //                | INTO | match(x)
                            // match (var) {  | ==== |
                            let a_value_clone = a_value.copy_as_ref();
                            let mut dummy_pending = HashSet::new();
                            dummy_pending.insert(*v);
                            let a_value_clone =
                                a_value_clone.commit_pending_variables(&dummy_pending);
                            if *a_variable == expr.0 && is_borrow_local(a_value_clone, *v) {
                                true
                            } else {
                                false
                            }
                        ...
                        if pop_last_ref {
                            codeunit.blocks.pop();
                            (
                                DecompiledExpr::EvaluationExpr(initial_evaluation_ctx.get_var(*v))
                                    .boxed(),
                                Some(RefType::ByValue),
                            )
                        } else {
                            (
                                DecompiledExpr::EvaluationExpr(expr.1).boxed(),
                                Some(RefType::ByValue),
                            )
                        }
                    }
                    ...
                    None => (DecompiledExpr::EvaluationExpr(expr.1).boxed(), None),
                };

                codeunit.add(DecompiledCodeItem::VariantSwitchStatement {
                    expr: match_expr,
                    cases_ref_type,
                    local_variable: expr.0,
                    cases: units,
                    result_variables,
                    use_as_result: ResultUsageType::None,
                });
        ...
        while let Some(bytecode) = iter.next() {
            ...
            match &bytecode.bytecode {
                ...
                Call(_, dsts, op, srcs, _) => {
                    ...
                    if let Some((name, fields, val, ref_type)) =
                        match &result.value().borrow().operation {
                            E::DatatypeUnpack(name, fields, val, types) => Some((
                                format!("{}{}", name, self.types_specifier_str(types)),
                                fields,
                                val,
                                None,
                            )),
                            E::VariantUnpack(name, vname, fields, val, types, ref_type) => Some((
                ...
                VariantSwitch(_, v, _) => {
                    let value = evaluation_ctx.get_var(*v);
                    if value.is_non_trivial() {
                        unreachable!("variant switch on non trivial condition");
                    }
                    evaluation_ctx.push_switch_variable(*v, value.copy())?;
                }
            }
        }
...
fn is_borrow_local(a_value_clone: Box<DecompiledExpr>, v: usize) -> bool {
    match a_value_clone.as_ref() {
        DecompiledExpr::EvaluationExpr(expr) => is_borrow_local_expr(expr, v),
        _ => false,
    }
}

fn is_borrow_local_expr(expr: &super::evaluator::stackless::expr_node::Expr, v: usize) -> bool {
    if let Some(expr) = expr.is_borrow() {
        if let Some(v2) = expr.is_local_variable() {
            return v == v2;
        }
    }
    false
...
fn get_case_var_ref_type(
    unit: &WithMetadata<CodeUnitBlock<usize, StacklessBlockContent>>,
) -> Option<(Option<usize>, move_model::model::RefType)> {
    let mut iter = unit.content_iter();
    // check for pattern
    // UnpackVariant(mid, dtid, vid, _, ref_type)
    // Assign(_, dst, src, kind)
    // Operation::UnpackVariant(mid, dtid, vid, [dst], ref_type2)
    // return (src, ref_type2)
    if let Some(block) = iter.next() {
        use move_stackless_bytecode::stackless_bytecode::Bytecode;
        let mut op_iter = block.content.code.iter().filter(|x| {
            !x.removed
                && !matches!(
                    x.bytecode,
                    Bytecode::Nop(_)
        ...
        let Some(bytecode) = op_iter.next() else {
            return None;
        };
        let Bytecode::Call(_, _, Operation::UnpackVariant(mid, dtid, vid, _, ref_type), _, _) =
            bytecode.bytecode
        else {
            return None;
        };
        if ref_type != move_model::model::RefType::ByImmRef {
            return Some((None, ref_type));
        }

        let Some(bytecode2) = op_iter.next() else {
            return Some((None, ref_type));
        };
        let Bytecode::Assign(
            _,
            dst,
            src,
            move_stackless_bytecode::stackless_bytecode::AssignKind::Move,
        ) = bytecode2.bytecode
        else {
            return Some((None, ref_type));
        };

        let Some(bytecode3) = op_iter.next() else {
            return Some((None, ref_type));
        };
        if let Bytecode::Call(
            _,
            _,
            Operation::UnpackVariant(mid2, dtid2, vid2, _, ref_type2),
            dsts,
            _,
        ) = &bytecode3.bytecode
        {
            if mid == *mid2 && dtid == *dtid2 && vid == *vid2 && *dsts == [dst] {
                return Some((Some(src), *ref_type2));
            } else {
                return Some((None, ref_type));
            }
        } else {
