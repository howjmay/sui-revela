...
pub enum HyperBlock<BlockIdentifier: BlockIdentifierTrait, BlockContent: BlockContentTrait> {
    ...
    VariantSwitchBlocks {
        cases: Vec<Box<WithMetadata<CodeUnitBlock<BlockIdentifier, BlockContent>>>>,
    },
}
...
impl<BlockIdentifier: BlockIdentifierTrait> Terminator<BlockIdentifier> {
    pub fn next_blocks(&self) -> Vec<&BlockIdentifier> {
        match self {
            ...
            Terminator::VariantSwitch { targets } => targets.iter().collect(),
...
impl<BlockIdentifier: BlockIdentifierTrait> Display for Terminator<BlockIdentifier> {
    fn fmt(&self, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Terminator::VariantSwitch { .. } => write!(fmt, "VariantSwitch"),
    ...
    HyperBlock<BlockIdentifier, BlockContent>
{
    pub fn content_iter_mut(
        ...
        match self {
            ...
            HyperBlock::VariantSwitchBlocks { cases } => {
                Box::new(cases.iter_mut().map(|x| x.content_iter_mut()).flatten())
            }
        }
    }
    ...
    pub fn content_iter(
        ...
        match self {
            ...
            HyperBlock::VariantSwitchBlocks { cases } => {
                Box::new(cases.iter().map(|x| x.content_iter()).flatten())
            }
        }
    }
            ...
            HyperBlock::VariantSwitchBlocks { cases } => {
                ...
                ctx.block(|ctx| {
                    for (idx, case) in cases.iter().enumerate() {
                        ctx.add_lines(&format!("case {}: {{", idx));
                        ctx.block(|ctx| {
                            case.inner().display(ctx);
                        });
                        ctx.add_lines("}");
                    }
                });
    ...
    pub fn is_abort(&self) -> bool {
        match self {
            ...
            HyperBlock::VariantSwitchBlocks { cases } => {
                cases.iter().all(|block| block.inner().is_abort())
            }
        }
    }
    ...
    pub fn is_terminated(&self) -> bool {
        match self {
            ...
            HyperBlock::VariantSwitchBlocks { cases } => {
                cases.iter().all(|block| block.inner().is_terminated())
            }
        }
    }
    ...
    pub fn is_terminated_in_loop(&self) -> bool {
        match self {
            ...
            HyperBlock::VariantSwitchBlocks { cases } => cases
                .iter()
                .all(|block| block.inner().is_terminated_in_loop()),
        }
    }
    ...
    pub fn terminator(&self) -> Option<&Terminator<BlockIdentifier>> {
        match self {
            ...
            HyperBlock::VariantSwitchBlocks { cases } => {
                cases.iter().find_map(|block| block.inner().terminator())
            }
        }
    }
}
