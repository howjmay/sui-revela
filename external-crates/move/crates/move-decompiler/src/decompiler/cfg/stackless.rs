        ...
        match block.inner_mut() {
            ...
            HyperBlock::VariantSwitchBlocks { cases } => {
                for case in cases.iter_mut() {
                    insert_terminated_blocks_if_needed(global_blocks, case, next_block)?;
                }
                next_block = None;
            }
        }
...
fn debug_dump_program(
    ...
    for block in program.blocks.iter().map(to_inner) {
        match block {
            ...
            HyperBlock::VariantSwitchBlocks { cases } => {
                println!("{}VariantSwitch", prefix);
                for (idx, case) in cases.iter().enumerate() {
                    println!("{}- Case {}", prefix, idx);
                    debug_dump_program(case.inner(), lvl + 1, show_bytecode);
                }
            }
        }
    }
...
fn trim_continue(
    ...
    while let Some(block) = iter.next() {
        ...
        match block {
            ...
            HyperBlock::VariantSwitchBlocks { cases: _ } => {
                // do nothing?
            }
        }
    }
...
fn is_empty_hyper_block(block: &WithMetadata<HyperBlock<usize, StacklessBlockContent>>) -> bool {
    match block.inner() {
        ...
        HyperBlock::VariantSwitchBlocks { .. } => false,
    }
}
    ...
    while let Some((from_block, to_block)) = {
        ...
        for block in blocks.iter_mut() {
            match block.next.clone() {
                ...
                Terminator::VariantSwitch { targets } => {
                    ...
                    block.next = Terminator::VariantSwitch { targets };
                }
...
fn trim_else(
    ...
    while let Some(block) = iter.next() {
        ...
        match block.inner_mut() {
            ...
            HyperBlock::VariantSwitchBlocks { cases } => {
                for case in cases.iter_mut() {
                    trim_else(case.as_mut(), in_loop, current_is_last_block_in_loop);
                }
                new_blocks.push(block.clone());
            }
        }
...
fn apply_short_circuit_jumps(
    ...
    for block in program.blocks.iter_mut() {
        match block.inner_mut() {
            ...
            HyperBlock::VariantSwitchBlocks { cases } => {
                for case in cases.iter_mut() {
                    apply_short_circuit_jumps(case);
                }
            }
        }
    }
            ...
            if let (true, Some((exit, _))) = (
                ...
                match node.next.clone() {
                    ...
                    Terminator::VariantSwitch { targets } => {
                        ...
                        flush(&mut chaining_blocks, &mut p);
                        let paths =
                            follow_variant_switch_boundaries(global_blocks, &mut iter, targets)?;
...
fn follow_variant_switch_boundaries(
    global_blocks: &[BasicBlock<usize, StacklessBlockContent>],
    iter: &mut core::slice::Iter<BasicBlock<usize, StacklessBlockContent>>,
    targets: Vec<usize>,
) -> Result<Option<WithMetadata<HyperBlock<usize, StacklessBlockContent>>>, anyhow::Error> {
    if targets.len() <= 1 {
        // if there is at most 1 target, just treat it as a normal branch
        return Ok(None);
    }
    let mut inner_paths: Vec<Vec<_>> = vec![Vec::new(); targets.len()];
    let mut current = usize::MAX;
    let targets_set: HashMap<usize, usize> = HashMap::from_iter(targets.iter().copied().zip(0..));
    // map of node => set of inner paths that can reach that node
    let mut reachable: HashMap<usize, HashSet<usize>> = HashMap::new();
    let mut join_nodes = HashSet::new();
    loop {
        let backup_iter = iter.clone();
        if let Some(next_block) = iter.next() {
            if join_nodes.contains(&next_block.idx) {
                *iter = backup_iter;
                break;
            }
            if let Some(target_idx) = targets_set.get(&next_block.idx) {
                if !inner_paths[*target_idx].is_empty() {
                    return Err(anyhow::anyhow!(
                        "Variant switch target {} is duplicated",
                        next_block.idx
                    ));
                }
                current = *target_idx;
            ...
            if current == usize::MAX {
                return Err(anyhow::anyhow!("Variant switch not started with a target"));
            }

            inner_paths[current].push(next_block.clone());
            let next_blocks = next_block.next.next_blocks();
            for &next in next_blocks {
                let entry = reachable.entry(next).or_insert_with(HashSet::new);
                entry.insert(current);
                if entry.len() > 1 {
                    join_nodes.insert(next);
                }
            }
        } else {
            break;
    ...
    Ok(Some(
        HyperBlock::VariantSwitchBlocks {
            cases: inner_paths
                .into_iter()
                .map(|nodes| Ok(Box::new(build_program(global_blocks, nodes.iter(), false)?)))
...
fn build_terminated_program(
    ...
    if global_blocks[target].next.is_terminated() {
        ...
        {
            let Some(if_program) = build_terminated_program(global_blocks, if_block.target) else {
                return None;
            };
            let Some(else_program) = build_terminated_program(global_blocks, else_block.target) else {
                return None;
            };
            let mut p = CodeUnitBlock {
                blocks: Vec::new(),
