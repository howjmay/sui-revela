    ...
    for (idx, block) in blocks.iter().enumerate() {
        ...
        match &block.next {
            Terminator::IfElse {
                ...
                edges[idx].push(if_block.target);
                edges[idx].push(else_block.target);
            }
    ...
    for (idx, &order_idx) in order.iter().enumerate() {
        ...
        block.next = match block.next {
            Terminator::IfElse {
                ...
                if_block: BranchTarget::unknown(rorder[if_block.target]),
                else_block: BranchTarget::unknown(rorder[else_block.target]),
            },
            ...
            Terminator::VariantSwitch { targets } => Terminator::VariantSwitch {
                targets: targets.iter().map(|&x| rorder[x]).collect(),
            },
        };
