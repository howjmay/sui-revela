    ...
    for (scc_idx, scc_nodes) in scc.sccs() {
        ...
        if must_be_exits.len() == 1 {
            ...
            if possible_exits.len() > 1 {
                if let Terminator::IfElse { else_block, .. } = bbs[scc_entry].next.clone() {
                    if possible_exits.contains(&else_block.target) {
                        scc_exit = else_block.target;
                    }
                }
                ...
                Terminator::IfElse {
                    ...
                    if b.idx != scc_entry {
                        b.next = Terminator::IfElse {
                            if_block: BranchTarget::unknown(add_dummy_block_if_required(i, if_block.target)),
                            else_block: BranchTarget::unknown(add_dummy_block_if_required(i, else_block.target)),
                        };
                    }
                }
                _ => {}
        ...
        if let Terminator::IfElse {
            if_block,
            else_block,
        } = bbs[scc_entry].next.clone()
        {
            if !scc_nodes.contains(&if_block.target) && if_block.target != scc_exit {
            ...
            if !is_valid_conditioned_entry {
                ...
                bbs[scc_entry].next = Terminator::IfElse {
                    if_block: BranchTarget::unknown(add_dummy_block_if_required(scc_entry, if_block.target)),
                    else_block: BranchTarget::unknown(add_dummy_block_if_required(scc_entry, else_block.target)),
                };
            }
        } else {
            is_valid_conditioned_entry = false;
