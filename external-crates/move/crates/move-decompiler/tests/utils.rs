...
use move_compiler::{diagnostics::WarningFilters, Flags};
...
pub(crate) fn into_binary_indexed_view<'a>(
    ...
    let mut binaries: Vec<CompiledModule> = Vec::new();

    binaries.extend(modules.iter().cloned());
...
pub(crate) fn tmp_project(tmp_files: Vec<(&str, &str)>, mut runner: impl FnMut(Vec<&str>)) {
    ...
    let project_root = tmp_dir.join(format!("decompiler--test-project-{}", uuid::Uuid::new_v4()));
