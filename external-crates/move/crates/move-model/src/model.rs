...
impl GlobalEnv {
    ...
    pub fn attach_compiled_module(
        ...
        for idx in 0..module.enum_defs.len() {
            let def_idx = EnumDefinitionIndex(idx as u16);
            let handle_idx = module.enum_def_at(def_idx).enum_handle;
            let handle = module.datatype_handle_at(handle_idx);
            let enum_id = DatatypeId(
                self.symbol_pool
                    .make(module.identifier_at(handle.name).as_str()),
            );
            let mod_data = &mut self.module_data[module_id.0 as usize];
            if let Some(_enum_data) = mod_data.enum_data.get_mut(&enum_id) {
                mod_data.enum_idx_to_id.insert(def_idx, enum_id);
            } else {
                panic!("attaching mismatching bytecode module")
            }
        }
    ...
    pub fn create_move_enum_data(
        ...
        let enum_smap = source_map
            .map(|smap| smap.get_enum_source_map(def_idx).ok())
            .flatten();
    ...
    pub fn globalize_signature(&self, sig: &SignatureToken) -> Type {
        match sig {
            ...
            SignatureToken::Datatype(handle_idx) => {
                ...
                let symbol = self.env.symbol_pool.make(sname);
                if let Some(struct_env) = declaring_module_env.find_struct(symbol.clone()) {
                    Type::Datatype(declaring_module_env.data.id, struct_env.get_id(), vec![])
                } else if let Some(enum_env) = declaring_module_env.find_enum(symbol) {
                    Type::Datatype(declaring_module_env.data.id, enum_env.get_id(), vec![])
                } else {
                    unreachable!("undefined struct or enum")
                }
            }
