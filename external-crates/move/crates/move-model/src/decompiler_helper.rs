...
pub fn run_stackless_compiler(
    ...
    for (module_count, (module_id, module_def)) in program.modules.into_iter().enumerate() {
        ...
        let dummy_compiled_module = create_dummy_compiled_module(
            &module_id,
            &module_def,
            move_core_types::account_address::AccountAddress::from_bytes(addr_bytes.into_bytes())
...
fn create_dummy_compiled_module(
    ...
    for (_loc, name, _edef) in &module_def.enums {
        let enum_handle = DatatypeHandle {
            module: ModuleHandleIndex(0),
            name: IdentifierIndex(identifiers.len() as u16),
            abilities: AbilitySet::EMPTY,
            type_parameters: vec![],
        ...
        let enum_def = EnumDefinition {
            enum_handle: DatatypeHandleIndex(datatype_handles.len() as u16),
            variants: vec![],
        };
        datatype_handles.push(enum_handle);
